...
export default async function ProfiloPage() {
  ...
  const [dbUser, eventi] = await Promise.all([
    prisma.user.findUnique({
      where:  { id: user.id },
      select: { email: true, nome: true, avatarUrl: true, isPremium: true, couponRiscattato: true, pianoAttivatoAt: true },
    }),
    prisma.event.findMany({
      ...
      select: {
        ...
        notificheVoto: true, digestNotifiche: true, avvisoDpp: true,
      },
    }),
  ]);
    ...
    <div className="px-4 md:px-6" style={{ paddingTop: 32, paddingBottom: 80, maxWidth: 680, margin: "0 auto" }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(44,44,46,0.50)", margin: "4px 0 0" }}>
          Foto, parametri evento, notifiche, piano e sicurezza
        </p>
      </div>
